                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="description" id="description" rows="6" required
                                    class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $course->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    ...
                                    <input type="number" name="estimated_duration" id="estimated_duration" value="{{ old('estimated_duration', $course->estimated_duration) }}" min="0"
                                        class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="language" id="language" value="{{ old('language', $course->language ?? 'English') }}"
                                        class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('language')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                            ...
                            @if($course->thumbnail)
                                <div class="mb-4">
                                    ...
                                    <img src="{{ Storage::url($course->thumbnail) }}" alt="{{ $course->title }}" class="h-32 w-auto rounded">
                                </div>
                            @endif
                            ...
                            <div class="mb-4">
                                <label for="thumbnail" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                    {{ $course->thumbnail ? 'Change Thumbnail' : 'Course Thumbnail' }}
                                </label>
                            ...
                            <div x-show="activeTab === 'content'" x-cloak>
                                ...
                                <div class="mb-4">
                                    ...
                                    <div id="objectives-container" class="space-y-2">
                                        @if($course->learning_objectives && is_array($course->learning_objectives))
                                            @foreach($course->learning_objectives as $index => $objective)
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" id="tags_input" placeholder="Enter tags separated by commas"
                                        value="{{ is_array($course->tags) ? implode(', ', $course->tags) : '' }}"
                                        class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="url" name="preview_video" id="preview_video" value="{{ old('preview_video', $course->preview_video) }}"
                                        class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                        placeholder="https://...">
                                    @error('preview_video')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="target_audience" id="target_audience" rows="3"
                                    class="w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('target_audience', $course->target_audience) }}</textarea>
                                ...
                                <select name="prerequisites[]" id="prerequisites" multiple
                                    ...
                                    @foreach($courses as $prereqCourse)
                                        <option value="{{ $prereqCourse->id }}" 
                                            {{ in_array($prereqCourse->id, old('prerequisites', $course->prerequisites ?? [])) ? 'selected' : '' }}>
                                            {{ $prereqCourse->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <a href="{{ route('admin.courses.show', $course) }}" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                Cancel
                            </a>
