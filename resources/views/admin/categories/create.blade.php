                    ...
                    <form method="POST" action="{{ route('admin.categories.store') }}">
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="description" 
                                      id="description" 
                                      rows="3"
                                      class="w-full rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-blue-500 focus:ring-blue-500">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <select name="parent_id" 
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('parent_id') == $category->id ? 'selected' : '' }}>
                                        {{ str_repeat('— ', $category->ancestors()->count()) }}{{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('parent_id')
